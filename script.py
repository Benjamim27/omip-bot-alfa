...
def procurar_preco_solar(texto):
    """Procura cirurgicamente o valor numérico à frente de 'SPEL SOLAR' no texto limpo"""
    padrao = r"SPEL SOLAR\s*(?:€)?\s*([\d.,]+)\s*(?:Eur/MWh)?"
...
def obter_dados_omip_validados():
    """Faz a leitura do texto corrido do site e organiza os blocos de Portugal, Espanha e o Spot Solar"""
    ...
    painel_spot = {
        "SPEL_SOLAR": ("SPEL SOLAR", 0.0)
    }
    
    try:
        resposta = requests.get(url, headers=headers, timeout=15)
        if resposta.status_code != 200:
            return painel_pt, painel_es, painel_spot
            
        texto_pagina = re.sub(r'<[^>]+>', ' ', resposta.text)
        texto_pagina = " ".join(texto_pagina.split())
        
        # ☀️ MERCADO SPOT SOLAR
        preco_solar = procurar_preco_solar(texto_pagina)
        painel_spot["SPEL_SOLAR"] = ("SPEL SOLAR", preco_solar)
...
def salvar_historico(data_envio, pt, es, spot):
    historico = {"DATA_ENVIO": data_envio, "PORTUGAL": pt, "ESPANHA": es, "SPOT": spot}
...
def enviar_email(dados_pt, dados_es, dados_spot, data_envio):
    ...
    <body style="font-family: Arial, sans-serif; color: #333; background-color: #f9f9f9; padding: 20px;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            ...
            <div style="padding: 20px;">
                ...
                <h3 style="color: #e67e22; border-bottom: 2px solid #e67e22; padding-bottom: 5px; margin-top: 25px;">☀️ MERCADO SPOT (Índice Diário)</h3>
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                    <tr style="background-color: #fdf5e6;">
                        <th style="padding: 12px 10px; border: 1px solid #ddd; text-align: left;">Índice</th>
                        <th style="padding: 12px 10px; border: 1px solid #ddd; text-align: right;">Preço</th>
                    </tr>
                    <tr>
                        <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; color: #d35400;">✨ {dados_spot['SPEL_SOLAR'][0]}</td>
                        <td style="padding: 10px; border: 1px solid #ddd; text-align: right; font-weight: bold; color: #d35400;">{dados_spot['SPEL_SOLAR'][1]:.2f} €/MWh</td>
                    </tr>
                </table>
...
if __name__ == "__main__":
    ...
    pt_atual, es_atual, spot_atual = obter_dados_omip_validados()
    
    # 🛑 TRAVA DE SEGURANÇA REFORÇADA: Se o Solar ou as Bases falharem totalmente, bloqueia a execução de erro
    if pt_atual["BASE"][1] == 0.0 or spot_atual["SPEL_SOLAR"][1] == 0.0:
        print("⚠️ [BLOQUEIO] O site do OMIP não devolveu preços válidos para o mercado a prazo ou Spot Solar.")
    ...
    if not historico_anterior:
        ...
        spot_velho = historico_anterior.get("SPOT", {})
        
        precos_pt_atual = extrair_apenas_precos(pt_atual)
        precos_es_atual = extrair_apenas_precos(es_atual)
        precos_spot_atual = extrair_apenas_precos(spot_atual)
        
        precos_pt_velho = extrair_apenas_precos(pt_velho)
        precos_es_velho = extrair_apenas_precos(es_velho)
        precos_spot_velho = extrair_apenas_precos(spot_velho)
        
        # O disparo ocorre se houver mudança nos prazos ou no Spot Solar
        if precos_pt_atual != precos_pt_velho or precos_es_atual != precos_es_velho or precos_spot_atual != precos_spot_velho:
            print("💰 Alteração real detetada nos preços de mercado!")
            houve_alteracao = True

    print("\n=============================================")
    print(f"🔍 VALORES LIDOS NESTE MOMENTO:")
    print(f"☀️ Solar Spot: {spot_atual['SPEL_SOLAR'][0]} -> {spot_atual['SPEL_SOLAR'][1]}€")
    print(f"🇵🇹 Portugal:   {pt_atual['BASE'][0]} -> {pt_atual['BASE'][1]}€")
    print(f"🇪🇸 Espanha:    {es_atual['BASE'][0]} -> {es_atual['BASE'][1]}€")
    print("=============================================\n")

    if houve_alteracao:
        if enviar_email(pt_atual, es_atual, spot_atual, momento_verificacao):
            salvar_historico(momento_verificacao, pt_atual, es_atual, spot_atual)
